...
export default function AddMaintanence() {
  ...
  const [allMaintanence, setAllMaintanence] = useState<any[] | null>(null);
  const [visibleMaintCount, setVisibleMaintCount] = useState(10);
  const [isdelmsg, setisdelmsg] = useState(false);
  const [yesloading, setyesloading] = useState(false);
  const [delMaintanenceData, setdelMaintanenceData] = useState({ id: "", name: "" });
  ...
  const handleDeleteMaintanenceMsg = (id: string, maintanenceName: string) => {
    setisdelmsg(true);
    setdelMaintanenceData({ id, name: maintanenceName });
  };
    ...
    <DashboardLayout userName={userData.name}>
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Add Maintenance</h1>
        ...
        {err && (
          <p className={`text-sm font-medium ${err === "Maintenance added" ? "text-green-500" : "text-red-500"}`}>
            {err}
          </p>
        )}
          ...
          <>
            ...
            <div className="space-y-3">
              {allMaintanence === null ? (
                ...
                <>
                {allMaintanence.slice().reverse().slice(0, visibleMaintCount).map((item: any) => (
                  <div
                    key={item._id}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-gray-900 dark:text-white">
                            {item.maintanence_name}
                          </p>
                          <p className="text-xs text-gray-500 dark:text-slate-400">
                            {item.maintanence_Day} - {item.maintanence_M_Y}
                          </p>
                        </div>
                      </div>
                    </div>
                    <span className="text-base font-bold text-purple-600 dark:text-purple-400 mr-3">
                      ₹{Number(item.maintanence_amount).toLocaleString("en-IN")}
                    </span>
                    <button
                      onClick={() => handleDeleteMaintanenceMsg(item._id, item.maintanence_name)}
                ...
                {visibleMaintCount < allMaintanence.length && (
                  <div className="flex justify-center pt-2">
                    <button
                      onClick={() => setVisibleMaintCount((prev) => prev + 10)}
                    ...
                    >
                      Load more ({allMaintanence.length - visibleMaintCount} remaining)
                    </button>
                  </div>
                )}
                </>
                ...
                <div className="py-10 text-center bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700">
                  <p className="text-gray-400 dark:text-slate-500 text-sm">No maintenance added yet</p>
                </div>
              )}
            </div>
          </>
      ...
      {isdelmsg && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-sm shadow-xl border border-gray-100 dark:border-slate-700">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white text-center">Delete Maintenance</h3>
            <p className="text-gray-500 dark:text-slate-400 text-center mt-2">
              Delete <span className="font-semibold text-gray-900 dark:text-white">{delMaintanenceData.name}</span>?
            </p>
